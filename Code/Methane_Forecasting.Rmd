...
# Recreate repeating seasonal pattern (fixed for classical decomposition)
seasonal_pattern <- head(methane_seasonality, 12)

# Repeat it to match 36 months
seasonality_future <- ts(rep(seasonal_pattern, 3),
                         start = c(2022, 1),
                         frequency = 12)

arima_forecast_seasoned <- ts(forecast_arima$mean + decomposed$seasonal[length(methane_train_ts) + 1:36],
  start = c(2022, 1),
  frequency = 12
)

# Add seasonality back into forecast
forecast_arima$mean <- forecast_arima$mean + seasonality_future
forecast_arima$lower <- forecast_arima$lower + seasonality_future
forecast_arima$upper <- forecast_arima$upper + seasonality_future
