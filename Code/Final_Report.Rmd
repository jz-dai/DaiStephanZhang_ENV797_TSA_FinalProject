...
The Top 1 method is the combination of 6 models:
a) ETS model
...
The residual plot from `checkresiduals()` shows that the residuals fluctuate randomly around zero. The ACF plot shows a significant spike at lag 1, but subsequent lags are mostly within the 95% confidence bounds. The residuals approximately follow a normal distribution, but there is slight skewness from the fitted normal curve.

```{r, fig.cap="Residual plot for best model 1"}
...
The Top 1 method is the combination of 7 models:
...
```{r, fig.cap="Residual plot for best model 2"}
...
The residual pattern is similar to Model 1, but there exist less randomness of the residual. In addition, the spike of lag 1 is higher, and the skewness from normal distribution is higher. These suggest a higher degree of autocorrelation.
...
The residuals of the SARIMAX model appear to imdicate that this is a good fit (Fig. 11). There is no trend in residuals time over. Although, there are some spikes. The ACF plot does not have any significant lags, indicating that autocorrelation is not prevalent. Finally, the histogram indicates that residuals are mostly normal with a slight skew - but nothing alarming. 
...
We used the SARIMAX model to predict temperature for 3 years into the future, using the whole data set as a training data set and assuming that the current climate scenario does not drastically change (Fig. 11). The future project appears to be strongly dominated by the seasonality of methane concentrations with a bias for the stable trend over the past couple of years. It does not seem likely that this forecast will accurately reflect the future temperatures. 
